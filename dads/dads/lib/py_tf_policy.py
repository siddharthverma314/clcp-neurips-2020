...
class PyTFPolicy(py_policy.Base, session_utils.SessionUser):
    ...
    def _construct(self, batch_size, graph):
        ...
        with graph.as_default():
            ...
            self._action_distribution = self._tf_policy.distribution(
                self._time_step, policy_state=self._policy_state
            ).action
            self._log_prob = common.log_probability(
                self._action_distribution, self._actions, self._tf_policy.action_spec
            )
    ...
    """
        ...
        if not self._built:
            raise RuntimeError("PyTFPolicy must be initialized before being restored.")
        if not graph:
            graph = tf.compat.v1.get_default_graph()
    ...
    def _build_from_time_step(self, time_step):
        outer_shape = nest_utils.get_outer_array_shape(time_step, self._time_step_spec)
