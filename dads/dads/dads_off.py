...
flags.DEFINE_integer(
    "reduced_observation", 0, "Predict dynamics in a reduced observation space"
)
flags.DEFINE_integer(
...
flags.DEFINE_float(
    "resample_prob", 0.0, "Creates stochasticity timesteps before resampling skill"
)
...
flags.DEFINE_integer(
    "record_freq", 100, "Video recording frequency within the training loop"
)
...
flags.DEFINE_integer(
    "deterministic_eval", 0, "Evaluate all skills, only works for discrete skills"
)
...
flags.DEFINE_string("skill_type", "cont_uniform", "Type of skill and the prior over it")
...
flags.DEFINE_integer(
    "replay_buffer_capacity", int(1e6), "Capacity of the replay buffer"
)
flags.DEFINE_integer(
...
# relabelling
flags.DEFINE_string(
    "agent_relabel_type", None, "Type of skill relabelling used for agent"
)
flags.DEFINE_integer(
...
flags.DEFINE_integer("debug_skill_relabelling", 0, "analysis of skill relabelling")

# skill dynamics optimization hyperparamaters
flags.DEFINE_integer(
    "skill_dyn_train_steps", 8, "Number of discriminator train steps on a batch of data"
)
flags.DEFINE_float(
    "skill_dynamics_lr", 3e-4, "Learning rate for increasing the log-likelihood"
)
flags.DEFINE_integer(
    "skill_dyn_batch_size", 256, "Batch size for discriminator updates"
)
# agent optimization hyperparameters
flags.DEFINE_integer("agent_batch_size", 256, "Batch size for agent updates")
flags.DEFINE_integer("agent_train_steps", 128, "Number of update steps per iteration")
...
flags.DEFINE_integer(
    "num_components", 4, "Number of components for Mixture of Gaussians"
)
flags.DEFINE_integer("fix_variance", 1, "Fix the variance of output distribution")
...
flags.DEFINE_float(
    "upright_threshold", 0.9, "Threshold before which the DKitty episode is terminated"
)
...
flags.DEFINE_float(
    "scale_root_position", 1, "Multiply the root coordinates the magnify the change"
)
flags.DEFINE_integer("run_on_hardware", 0, "Flag for hardware runs")
flags.DEFINE_float(
    "randomize_hfield", 0.0, "Randomize terrain for better DKitty transfer"
)
flags.DEFINE_integer(
    "observation_omission_size", 2, "Dimensions to be omitted from policy input"
)

# Manipulation Environments
flags.DEFINE_integer(
    "randomized_initial_distribution", 1, "Fix the initial distribution or not"
)
flags.DEFINE_float(
    "horizontal_wrist_constraint",
    1.0,
    "Action space constraint to restrict horizontal motion of the wrist",
)
flags.DEFINE_float(
    "vertical_wrist_constraint",
    1.0,
    "Action space constraint to restrict vertical motion of the wrist",
)

# MPC hyperparameters
flags.DEFINE_integer(
    "planning_horizon", 1, "Number of primitives to plan in the future"
)
flags.DEFINE_integer("primitive_horizon", 1, "Horizon for every primitive")
flags.DEFINE_integer(
    "num_candidate_sequences",
    50,
    "Number of candidates sequence sampled from the proposal distribution",
)
...
flags.DEFINE_string(
    "prior_type", "normal", "Uniform or Gaussian prior for candidate skill(s)"
)
flags.DEFINE_float("smoothing_beta", 0.9, "Smooth candidate skill sequences used")
flags.DEFINE_integer(
    "top_primitives", 5, "Optimization parameter when using uniform prior (CEM style)"
)
...
def get_environment(env_name="point_mass"):
    ...
    if env_name == "Ant-v1":
        env = ant.AntEnv(expose_all_qpos=True, task="motion")
        observation_omit_size = 2
    elif env_name == "Ant-v1_goal":
        observation_omit_size = 2
        return wrap_env(
            ant.AntEnv(task="goal", goal=goal_coord, expose_all_qpos=True),
...
def relabel_skill(
    trajectory_sample, relabel_type=None, cur_policy=None, cur_skill_dynamics=None
):
    global observation_omit_size
    if relabel_type is None or (
        "importance_sampling" in relabel_type and FLAGS.is_clip_eps <= 1.0
    ):
    ...
    time_steps, action_steps, next_time_steps = nest.map_structure(
        lambda t: np.squeeze(t, axis=1), (time_steps, action_steps, next_time_steps)
    )
    ...
    if "importance_sampling" in relabel_type:
        ...
        for idx in range(time_steps.observation.shape[0]):
            ...
            cur_log_prob = cur_policy.log_prob(
                cur_time_step, action_steps.action[idx : idx + 1]
            )[0]
            is_weights.append(
                np.clip(
                    np.exp(cur_log_prob - old_log_prob),
                    1.0 / FLAGS.is_clip_eps,
                    FLAGS.is_clip_eps,
                )
            )
    ...
    for idx in range(time_steps.observation.shape[0]):
        alt_time_steps = nest.map_structure(
            lambda t: np.stack([t[idx]] * FLAGS.num_samples_for_relabelling), time_steps
        )
        ...
        if relabel_type == "policy":
            cur_action = np.stack(
                [action_steps.action[idx, :]] * FLAGS.num_samples_for_relabelling
            )
            alt_time_steps = alt_time_steps._replace(
                observation=np.concatenate(
                    [
                        alt_time_steps.observation[
                            :, observation_omit_size : -FLAGS.num_skills
                        ],
                        alt_skills,
                    ],
                    axis=1,
            ...
            if FLAGS.debug_skill_relabelling:
                print(
                    "\n action_log_probs analysis----",
                    idx,
                    time_steps.observation[idx, -FLAGS.num_skills :],
                )
                print(
                    "number of skills with higher log-probs:",
                ...
                print(
                    "average distance of skills with higher-log-prob:",
                    np.mean(skill_dist),
                )
        ...
        # choose the skill which gets the highest log-probability under the dynamics posterior
            ...
            posterior_log_probs = cur_skill_dynamics.get_log_prob(
                process_observation(cur_observations),
                alt_skills,
                process_observation(next_observations),
            )
            if FLAGS.debug_skill_relabelling:
                print(
                    "\n dynamics_log_probs analysis----",
                    idx,
                    time_steps.observation[idx, -FLAGS.num_skills :],
                )
                print(
    ...
    new_trajectory_sample = trajectory_sample._replace(observation=traj_observation)
...
def process_observation(observation):
    ...
    # x-y plane
        ...
        if (
            "Ant" in FLAGS.environment
            or "DKitty" in FLAGS.environment
            or "DClaw" in FLAGS.environment
        ):
            red_obs = [
    ...
    # (x, y, orientation), works only for ant, point_mass
        ...
        # x, y, z of the center of the block
            ...
            red_obs = [
                _shape_based_observation_processing(
                    observation, observation.shape[-1] - 7
                ),
                _shape_based_observation_processing(
                    observation, observation.shape[-1] - 6
                ),
                _shape_based_observation_processing(
                    observation, observation.shape[-1] - 5
                ),
            ]
...
def collect_experience(py_env, time_step, collect_policy, buffer_list, num_steps=1):
    ...
    for step_idx in range(num_steps):
        ...
        if FLAGS.action_clipping < 1.0:
            action_step = action_step._replace(
                action=np.clip(
                    action_step.action, -FLAGS.action_clipping, FLAGS.action_clipping
                )
            )
        ...
        if (
            ...
            action_step = action_step._replace(
                info=policy_step.set_log_probability(
                    action_step.info, cur_action_log_prob
                )
            )
    ...
    return (
        time_step,
        {"episode_sizes": episode_sizes, "episode_return": extrinsic_reward},
    )
        ...
        if dynamics is not None:
            if FLAGS.reduced_observation:
                cur_observation, next_observation = (
                    process_observation(cur_observation),
                    process_observation(next_observation),
                )
            logp = dynamics.get_log_prob(
                np.expand_dims(cur_observation, 0),
                np.expand_dims(cur_skill, 0),
            ...
            for _ in range(predict_trajectory_steps):
                next_predicted_state = dynamics.predict_state(
                    cur_predicted_state, skill_expanded
                )
...
def eval_loop(eval_dir, eval_policy, dynamics=None, vid_name=None, plot_name=None):
    ...
    for idx in range(num_evals):
        if FLAGS.num_skills > 0:
            if FLAGS.deterministic_eval:
                ...
                preset_skill = np.random.multinomial(
                    1, [1.0 / FLAGS.num_skills] * FLAGS.num_skills
                )
        ...
        if vid_name is not None:
            ...
            eval_env = video_wrapper.VideoWrapper(
                eval_env, base_path=eval_dir, base_name=full_vid_name
            )
        ...
        for eval_idx in range(per_skill_evaluations):
            eval_trajectory = run_on_env(
                ...
                return_data=True,
                close_environment=True
                if eval_idx == per_skill_evaluations - 1
                else False,
            )
            ...
            if plot_name is not None:
                ...
                if predict_trajectory_steps > 0:
                    for step_idx in range(len(eval_trajectory)):
                        if step_idx % 20 == 0:
                            plt.plot(
                                eval_trajectory[step_idx][-1][:, 0],
                                eval_trajectory[step_idx][-1][:, 1],
                            )
    ...
    **kwargs
    ...
    for _ in range(episode_horizon // primitive_horizon):
        ...
        running_cur_state = np.array(
            [process_observation(time_step.observation)] * latent_action_space_size
        )
        ...
        for _ in range(primitive_horizon):
            ...
            next_time_step = env.step(
                np.clip(
                    policy.action(
                        hide_coords(
                            time_step._replace(observation=skill_concat_observation)
                        )
                    ).action,
  ...
  """
    ...
    def _smooth_primitive_sequences(primitive_sequences):
        for planning_idx in range(1, primitive_sequences.shape[1]):
            primitive_sequences[:, planning_idx, :] = (
                smoothing_beta * primitive_sequences[:, planning_idx - 1, :]
                + (1.0 - smoothing_beta) * primitive_sequences[:, planning_idx, :]
            )
    ...
    def _get_init_primitive_parameters():
        if prior_type == "normal":
            ...
            prior_low = lambda: np.array([-1.0] * latent_action_space_size)
            prior_high = lambda: np.array([1.0] * latent_action_space_size)
    ...
    for _ in range(episode_horizon // primitive_horizon):
        for _ in range(refine_steps):
            ...
            running_cur_state = np.array(
                [process_observation(time_step.observation)] * num_candidate_sequences
            )
            ...
            for planning_idx in range(planning_horizon):
                ...
                for _ in range(primitive_horizon):
                    predicted_next_state = dynamics.predict_state(
                        running_cur_state, cur_primitives
                    )

                    # update running stuff
                    dense_reward = env.compute_reward(
                        running_cur_state, predicted_next_state
                    )
            ...
            _update_parameters(
                candidate_primitive_sequences, running_reward, primitive_parameters
            )
    ...
    with tf.compat.v2.summary.record_if(True):
        ...
        with tf.compat.v1.Session().as_default() as sess:
            ...
            if FLAGS.run_train:
                ...
                episode_return_buffer.append(0.0)
                ...
                if iter_count == 0:
                    ...
                    time_step, collect_info = collect_experience(
                        ...
                        collect_policy,
                        buffer_list=[rbuffer]
                        if not FLAGS.train_skill_dynamics_on_policy
                        else [rbuffer, on_buffer],
                        num_steps=FLAGS.initial_collect_steps,
                    )
                    _process_episodic_data(
                        episode_size_buffer, collect_info["episode_sizes"]
                    )
                    _process_episodic_data(
                        episode_return_buffer, collect_info["episode_return"]
                    )
                ...
                while iter_count < FLAGS.num_epochs:
                    ...
                    # model save
                    if (
                        FLAGS.save_model is not None
                        and iter_count % FLAGS.save_freq == 0
                    ):
                        print("Saving stuff")
                        ...
                        np.save(
                            os.path.join(log_dir, "episode_size_buffer"),
                            episode_size_buffer,
                        )
                        np.save(
                            os.path.join(log_dir, "episode_return_buffer"),
                            episode_return_buffer,
                        )
                    ...
                    print(
                        "intermediate time:", collect_start_time - agent_end_train_time
                    )
                    time_step, collect_info = collect_experience(
                        py_env,
                        time_step,
                    ...
                    print(
                        "Iter collection time:", collect_end_time - collect_start_time
                    )
                    ...
                    for _ in range(
                        ...
                        cur_skill = trajectory_sample.observation[
                            :, 0, -FLAGS.num_skills :
                        ]
                    ...
                    for _ in range(
                        ...
                        sampling_time_arr.append(
                            buffer_sampling_time - within_agent_train_time
                        )
                        trajectory_sample = _filter_trajectories(trajectory_sample)
                        ...
                        if (
                            ...
                            in FLAGS.skill_dynamics_relabel_type
                        ):
                            trajectory_sample = trajectory_sample._replace(
                                policy_info=()
                            )
                        ...
                        within_agent_train_time = time.time()
                        train_time_arr.append(
                            within_agent_train_time - relabelling_time
                        )
                        ...
                        tf.compat.v1.Summary(
                            value=[
                                tf.compat.v1.Summary.Value(
                                    tag="dads/logp_altz",
                                    simple_value=np.mean(
                                        np.concatenate(running_logp_altz)
                                    ),
                                )
                            ]
                        ),
                    ...
                    if FLAGS.clear_buffer_every_iter:
                        ...
                        episode_return_buffer = [0.0]

                    # within train loop evaluation
                    if (
                        FLAGS.record_freq is not None
                        and iter_count % FLAGS.record_freq == 0
                    ):
                ...
                print(
                    "\ttotal time for %d epochs: %f"
                    % (FLAGS.num_epochs, time.time() - meta_start_time)
                )
            ...
            if FLAGS.run_eval:
                ...
                image = tf.io.gfile.GFile(
                    os.path.join(eval_dir, save_label + ".png"), "w"
                )
                image.write(buf.read(-1))
                plt.clf()
