...
class PointMassEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(
        ...
        self._wiggle_direction = +1 if wiggly_weight > 0.0 else -1
    ...
    def step(self, action):
        ...
        if self._wiggly_weight > 0.0:
            reward = (np.exp(-((-reward) ** 0.5)) ** (1.0 - self._wiggly_weight)) * (
    ...
    def reset_model(self):
        qpos = self.init_qpos + np.append(
            self.np_random.uniform(low=-0.2, high=0.2, size=2),
            self.np_random.uniform(-np.pi, np.pi, size=1),
        )
        qvel = self.init_qvel + self.np_random.randn(self.sim.model.nv) * 0.01
        if self._sample_target is not None:
