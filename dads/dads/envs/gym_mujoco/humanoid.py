...
class HumanoidEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(
        self, expose_all_qpos=False, model_path="humanoid.xml", task=None, goal=None
    ):
    ...
    def compute_reward(self, ob, next_ob, action=None):
        ...
        if self._task == "forward":
            ...
            reward = (
                -np.linalg.norm(np.array([xposafter, yposafter]).T - self._goal, axis=1)
                > -0.3
            ).astype(np.float32)
        return reward
    ...
    def reset_model(self):
        ...
        self.set_state(
            self.init_qpos
            + self.np_random.uniform(low=-c, high=c, size=self.sim.model.nq),
            self.init_qvel
            + self.np_random.uniform(low=-c, high=c, size=self.sim.model.nv,),
        )
